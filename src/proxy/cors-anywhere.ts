...
import url from "url";
import { decodeBuffer } from "../core/encoding";
import { reportEvent } from "../core/reportEvent";
...
function showUsage(help_file, headers, response) {
  ...
  if (help_text[help_file] != null) {
    ...
    fs.readFile(help_file, "utf8", function (err, data) {
      if (err) {
...
async function proxyRequest(req, res, proxy) {
  ...
  // Listen for data events to capture chunks of the request body
  req.on("data", (chunk) => {
    requestBodyChunks.push(chunk);
  });

  // store headers to remove
  const storeHeaders = {};
  const headersToRemove = ["store-user-id", "store-user-address"];
  headersToRemove.forEach((header) => {
    storeHeaders[header] = req.headers[header];
        ...
        proxyReq.on = function (eventName, listener) {
          ...
          return proxyReqOn.call(this, "response", function (proxyRes) {
            ...
            proxyRes.on("end", async function () {
              const buffer = Buffer.concat(bodyChunk);
              const contentEncoding = proxyRes.headers?.["content-encoding"];
              const body = decodeBuffer(buffer, contentEncoding);

              const response: CachedProxyResponse = {
...
function getPathWithoutQueryParams(url) {
  return url.split("?")[0];
}

/**
 * @dev
 * To prevent abuse, only allow whitelisted URLs to be proxied.
 */
function isUrlWhitelisted(url, method) {
  if (!(method in whitelist)) return false;

  url = getPathWithoutQueryParams(url);

  for (const regexp of whitelist[method]) {
    if (regexp.test(url)) {
      return true;
    }
  }
...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    if (!isUrlWhitelisted(url, method)) {
      ...
      res.end(`The url ${url} is not allowed by the operator of this proxy.`);
      return;
    }
    ...
    proxyRequest(req, res, proxy).catch(function (err) {
      // eslint-disable-next-line no-console
      console.error("proxyRequest catch: Error in proxyRequest:", err);
      if (!res.headersSent) {
        res.writeHead(500);
        res.end("Internal server error");
      }
...
export function corsAnywhereMiddleware(options) {  
  ...
  return function (req, res, next) {
    ...
    if (req.url.startsWith("/commit")) {
      ...
      // Use the CORS Anywhere logic for other paths
      try {
        requestHandler(req, res);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error("requestHandler catch: Error in requestHandler:", error);
        if (!res.headersSent) {
          res.writeHead(500);
          res.end("Internal server error");
        }
